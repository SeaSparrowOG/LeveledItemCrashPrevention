...
namespace LeveledListUtils
{
	...
	template <typename T>
	inline void Audit() {
		static bool truncateLists = Settings::INI::GetSetting<bool>(
			Settings::INI::GENERAL_STATIC_SANITIZATION.data()
		).value_or(false);
		...
		std::vector<RE::TESLeveledList*> circularLists;
		std::unordered_set<RE::TESLeveledList*> audited;
		for (auto* list : lists) {
			...
			if (truncateLists && entries.size() > 255u) {
				entries.resize(255u);
			}
			for (auto it = entries.begin(); !circular && it != entries.end(); ++it) {
				auto* entryList = it->form ? skyrim_cast<RE::TESLeveledList*>(it->form) : nullptr;
